...
def find_k_nearest_neighbors(video_id_batch, video_batch, reader, data_pattern, batch_size=8192, k=3, debug=False):
    ...
    with tf.Graph().as_default() as graph:
        ...
        batch_topk_sims, batch_topk_sim_indices = tf.nn.top_k(similarities, k=_k)
...
def how_long_build_topk_graph():
    """
    Test how long it takes to build top k graph.
    :return: None. Print time.
    """
    ...
    model_type, feature_names, feature_sizes = FLAGS.model_type, FLAGS.feature_names, FLAGS.feature_sizes
    reader = get_reader(model_type, feature_names, feature_sizes)
    video_id_batch, video_batch, _, _ = get_input_data_tensors(reader, train_data_pattern, batch_size)
    ...
    try:
        while not coord.should_stop():
            ...
            with tf.Graph().as_default() as graph:
                ...
                video_id_batch_inner, video_batch_inner, video_labels_batch_inner, num_frames_batch_inner = (
                    get_input_data_tensors(
                        reader=inner_reader, data_pattern=train_data_pattern, batch_size=batch_size,
                        num_readers=num_readers, num_epochs=1, name_scope='inner_loop'))

                # normalization
                feature_dim = len(video_batch_inner.get_shape()) - 1

                video_batch_normalized = tf.nn.l2_normalize(video_batch_val, feature_dim)
            ...
            print('Building graph needs {} s'.format(end_time - start_time))
            coord.request_stop()
